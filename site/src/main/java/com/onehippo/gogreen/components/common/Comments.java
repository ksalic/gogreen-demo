...
public class Comments extends DocumentComponent {
    ...
    public void doBeforeRender(HstRequest request, HstResponse response) {
        ...
        HippoFolder commentsFolder = ctx.getSiteContentBaseBean().getBean(commentsFolderPath);
        log.debug("Comments folder " + commentsFolder);
        if (commentsFolder != null) {
            HippoDocumentBean document = (HippoDocumentBean) ctx.getContentBean();
            if (document != null) {
                try {
                    List<Comment> comments = new LinkedList<Comment>();

                    HstQuery incomingBeansQuery = ContentBeanUtils.createIncomingBeansQuery(document, commentsFolder, PATH_DEPTH,
                            Comment.class, false);
    ...
    private String getCommentsFolderPath(HstRequest request) {
        String commentsFolderPath = getComponentParameter(PARAM_COMMENTS_FOLDER);
    ...
    public void doAction(HstRequest request, HstResponse response) {
        ...
        String antiSpamInput = GoGreenUtil.getEscapedParameter(request, ANTISPAMINPUT);
        String antiSpamText = GoGreenUtil.getEscapedParameter(request, ANTISPAMTEXT);

        List<String> errors = new ArrayList<String>();

        if (!StringUtils.isEmpty(antiSpamInput) || !StringUtils.isEmpty(antiSpamText)) {
            errors.add("invalid.spam-label");
        }
        ...
        final HstRequestContext ctx = request.getRequestContext();
        HippoBean commentFor = ctx.getContentBean();
    ...
    private static class CommentsWorkflow extends BaseWorkflowCallbackHandler<DocumentWorkflow> {
        ...
        @Override
        public void processWorkflow(DocumentWorkflow workflow) throws RemoteException, RepositoryException,
                WorkflowException {
