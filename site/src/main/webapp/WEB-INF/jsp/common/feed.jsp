...
<c:choose>
  ...
  <c:when test="${numberOfItems eq 2}">
      <c:set var="colSize" value="6"/>
  </c:when>
  <c:when test="${numberOfItems eq 3}">
      <c:set var="colSize" value="4"/>
  </c:when>
  <c:when test="${numberOfItems ge 4}">
...
<div class="container">
    ...
    <div class="row rss-feed">
          <c:forEach items="${feed.entries}" var="entry" end="${numberOfItems - 1}" varStatus="status">
                      ...
                      <div class="feature-content">
                          <c:if test="${not empty icon}">
                              <div class="content-style3">
                                  <i class="fa content-style3-icon <c:out value="${icon}"/>"></i>
                              </div>
                          </c:if>
                          <p class="link">
