...
$(document).ready(function(){
    ...
    var exists = function(objectDescriptor) {
        if (!objectDescriptor) {
            return false;
        }
        var objects = objectDescriptor.split('.');
        var scope = window;
        var testScope = objects.shift();
        while (typeof scope[testScope] !== 'undefined') {
            if (objects.length === 0) {
                return true;
            }
            scope = scope[testScope];
            testScope = objects.shift();
        }
        return false;
    };
  ...
  $("#show-more a.more").click(function() {
    ...
    $.get(type+".ajax?jsEnabled=true&pageSize="+ pageSize + "&from=" + from + add, function(html) {
        ...
        if (exists('Hippo.ChannelManager.TemplateComposer.IFrame.Main')) {
            Hippo.ChannelManager.TemplateComposer.IFrame.Main.publish('dynamicContentLoaded');
        }
    });
    return false;
  });
