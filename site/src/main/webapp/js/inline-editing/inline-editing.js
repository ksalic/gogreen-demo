...
function init_inline_editor(editableElementsContainerName, editorFormName, richEditorTextAreaName, plainEditorTextAreaName, toolbarContainerName, surfAndEditLink) {
	// FIXME make this optional
	var darkMaskEnabled = false;
    ...
    myEditor.on('toolbarLoaded', function() {
    	...
    	this.toolbar.on('saveClick', function(o) { 
            ...
            myEditor.saveHTML();
            save_inline_editor(editorFormName);
    	...
    	this.toolbar.on('editInCMSClick', function(o) {
            ...
            window.open(surfAndEditLink, 'cms');
            close_inline_editor();
        }, myEditor, true);
...
var handleSuccess_inline_editor = function(o) {
    if(o.responseText !== undefined) {
        ...
        if (result.success)
        	showFeedback_inline_editor("Content saved.");
        else
        	//showFeedback(result.message);
        	showFeedback_inline_editor("Error!");
    }
};

var handleFailure_inline_editor = function(o) {
    if(o.responseText !== undefined){
    	showFeedback_inline_editor("Could not save content. Ajax request failed. HTTP status: " + o.status);
    }
};

var callback_inline_editor = {
    success:handleSuccess_inline_editor,
    failure:handleFailure_inline_editor,
    argument:['foo','bar']
};

function save_inline_editor(editorFormName) {
    var formObject = document.getElementById(editorFormName); 
    YAHOO.util.Connect.setForm(formObject);     
    var sUrl = formObject.action;
    var request = YAHOO.util.Connect.asyncRequest('POST', sUrl, callback_inline_editor);
...
function showFeedback_inline_editor(message) {
